...
public class TestActiviti {
	...
	//10.网关 - 并行网关(会签) - 项目经理和财务经理都审批�?,流程结束;如果只有�?个经理审�?,流程�?要等�?.
	@Test
	public void test101(){
		ProcessDefinition processDefinition = processEngine.getRepositoryService().createProcessDefinitionQuery().latestVersion().singleResult();
	...
	//8.流程变量
	//如果存在流程变量,那么,在启动流程实例时,要给流程变量赋�??.否则,启动流程实例会报�?.
	...
	public void test03(){
		...
		List<ProcessDefinition> list = processDefinitionQuery.list(); //查询�?有流程定�?
		...
		long count = processDefinitionQuery.count(); //查询流程定义记录�?
		...
		System.out.println("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%");
		//根据流程定义�? Key 查询流程定义对象
		ProcessDefinition processDefinition2 = processDefinitionQuery.processDefinitionKey("auth  ").latestVersion().singleResult();
		System.out.println("Id="+processDefinition2.getId());
		System.out.println("Key="+processDefinition2.getKey());
		System.out.println("Name="+processDefinition2.getName());
		System.out.println("Version="+processDefinition2.getVersion());
